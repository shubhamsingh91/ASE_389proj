...
class WBC(object):
    ...
    def solve(self, task_list, contact_list, verbose=False):
        ...
        for i, task in enumerate(task_list):
            ...
            cost_t_mat += w * np.matmul(j.transpose(), j)
            cost_t_vec += w * ((2 * np.matmul(j_dot_q_dot.transpose(), j)) - (2 * np.matmul(x_ddot.transpose(), j)))
        ...
        if contact_list is not None:
            ...
            eq_mat = np.concatenate((self._mass_matrix, -1*contact_jacobian.transpose()), axis=1)[0:6, :]

        else:
            eq_mat = np.dot(self._sf, self._mass_matrix)
        # TODO : Construct floating base dynamics constraint vector
        ##getting first 6 elements
        eq_vec = np.zeros(6) - self._coriolis[0:6] - self._gravity[0:6]
